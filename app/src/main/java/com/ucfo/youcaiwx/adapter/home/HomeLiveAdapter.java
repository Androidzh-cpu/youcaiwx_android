...
public class HomeLiveAdapter extends BaseAdapter<HomeBean.DataBean.BroadcastBean, HomeLiveAdapter.ViewHolder> {
    ...
    protected void onBindDataViewHolder(ViewHolder holder, int position) {
        ...
        if (!TextUtils.isEmpty(app_image)) {
            Glide.with(context).load(app_image).transform(new CenterCrop(context), new GlideRoundTransform(context,6))
                    .diskCacheStrategy(DiskCacheStrategy.ALL).crossFade().into(holder.mLiveImageItem);
        }
